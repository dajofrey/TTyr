...
static TTYR_CORE_RESULT tk_core_handleDeletedNodes(
    tk_core_FileEditor *Editor_p, tk_core_TreeListingNode *Node_p)
{
//#ifdef __unix__
//
...
//            TTYR_CHECK(tk_core_handleDeletedNodes(Editor_p, Child_p, updated_p))
//        }
//    }
//
//    if (namelist_pp) {
...
TTYR_CORE_RESULT tk_core_updateEditor(
    tk_core_Program *Program_p)
{
    ...
    TTYR_CHECK(tk_core_handleDeletedNodes(&Editor_p->FileEditor, Editor_p->TreeListing.Root_p))
...
static TTYR_CORE_RESULT tk_core_handleEditorInput(
    tk_core_Program *Program_p, nh_api_WSIEvent Event)
{
    ...
    switch (c) 
    {
        ...
        case 27 :
            ...
            TTYR_CHECK(tk_core_setDefaultMessage(
        ...
        case 'g' : if (Editor_p->insertMode) {goto FILE_EDITOR_INPUT;}
            ...
            TTYR_CHECK(tk_core_cycleThroughFiles(Program_p, c))
            Program_p->refresh = true;
            break;
        ...
        case 'd' : if (Editor_p->insertMode) {goto FILE_EDITOR_INPUT;}
            if  (Editor_p->treeListing) {
                TTYR_CHECK(tk_core_handleTreeListingInput(Program_p, c))
            }
...
FILE_EDITOR_INPUT :
    ...
    TTYR_CHECK(tk_core_handleFileEditorInput(Program_p, c))
    return TTYR_CORE_SUCCESS;
}

// DRAW ============================================================================================

static TTYR_CORE_RESULT tk_core_drawEditor(
    tk_core_Program *Program_p, tk_core_Glyph *Glyphs_p, int width, int height, int row)
{
    ...
    if (Editor_p->treeListing) {
        TTYR_CHECK(tk_core_drawTreeListingRow(Program_p, Glyphs_p, width, height, row))
        Glyphs_p += Editor_p->View.TreeListing.width;
    }
    
    // Draw file editor row.
    TTYR_CHECK(tk_core_drawFileEditorRow(
...
static TTYR_CORE_RESULT tk_core_drawEditorTopbar(
...
static TTYR_CORE_RESULT tk_core_getEditorCursor(
...
static TTYR_CORE_RESULT tk_core_executeEditorCommand(
    tk_core_Program *Program_p)
{
    tk_core_Editor *Editor_p = Program_p->handle_p;
...
//
//            TTYR_CHECK(tk_core_setDefaultMessage(
//                &tk_core_getTTY()->Tab_p->Tile_p->Status, TTYR_CORE_MESSAGE_EDITOR_NEW_FILE
//            ))
//
//            Program_p->refresh = true;
...
//                    TTYR_CHECK(tk_core_clearFileSearch(File_p))
//                }
//                else {
//                    nh_encoding_UTF32String *Argument_p = Arguments_p->pp[0];
//                    TTYR_CHECK(tk_core_searchFile(File_p, Argument_p->p, Argument_p->length))
//                }
//            }
//            break;
//        }
...
//
//            TTYR_CHECK(tk_core_setCustomMessage(
//               &tk_core_getTTY()->Tab_p->Tile_p->Status, TTYR_CORE_MESSAGE_EDITOR_NUMBER_OF_TAB_SPACES,
