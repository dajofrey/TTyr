...
//        TK_CHECK(tk_core_appendCanvasTile(&Tile_p->Canvas, Prototype_p, &TTY_p->Views, once))
//
    ...
    for (int i = 0; i < Tiles.size; ++i) {
        ...
        for (int j = 0; j < 9; ++j) {
            TK_CHECK(tk_core_appendMicroTab(
                &TK_CORE_MACRO_TAB_2(Tile_p, j)->MicroWindow, TTY_p->Prototypes.pp[TTY_p->Prototypes.size - 1], once
            ))
        }
    }
