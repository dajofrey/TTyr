...
static TTYR_CORE_RESULT ttyr_core_getMaxCursorPosition(
    ...
    int *cols_p, int *rows_p)
{
    ...
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
        ...
        TTYR_CHECK(ttyr_core_getMaxCursorPosition(&ws.ws_row, &ws.ws_col))
    } 
...
static TTYR_CORE_RESULT ttyr_core_readLinuxStandardInput(
...
#ifdef __unix__
    TTYR_CHECK(ttyr_core_readLinuxStandardInput(codepoints_p, &count))
#endif
...
static TTYR_CORE_RESULT ttyr_core_enterRawMode(
...
static TTYR_CORE_RESULT ttyr_core_exitRawMode(
    ttyr_core_View *View_p) 
{
#ifdef __unix__
    ...
    ttyr_core_TTY *TTY_p)
{
    ...
    ttyr_core_View *View_p = ttyr_core_createView(TTY_p, NULL, true);
    TTYR_CHECK_NULL(View_p)

    TTYR_CORE_RESULT error = ttyr_core_enterRawMode(View_p);
    if (error) {
        ttyr_core_destroyView(TTY_p, View_p);
        return error;
    }
    ...
    ttyr_core_TTY *TTY_p)
{
    ...
    TTYR_CHECK(ttyr_core_exitRawMode(View_p))
    ttyr_core_destroyView(TTY_p, View_p);
...
{
    for (int i = 0; i < TTY_p->Views.size; ++i) {
        if (((ttyr_core_View*)TTY_p->Views.pp[i])->standardIO) {return true;}
    }
    return false;
}
