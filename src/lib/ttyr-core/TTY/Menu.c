    ...
    ttyr_core_Row *Grid_p)
{
    ...
    // Get menu height.
    int height = TTYR_CORE_MACRO_TILE(TTY_p->Window_p->Tile_p)->MacroTabs.size+2;
    if (height <= 0) {return TTYR_CORE_SUCCESS;}

    // Get menu width.
    int width = 0;
    for (int i = 0; i <  TTYR_CORE_MACRO_TILE(TTY_p->Window_p->Tile_p)->MacroTabs.size; ++i) {
        ttyr_core_Program *Program_p = ttyr_core_getCurrentProgram(&((ttyr_core_MacroTab*)((ttyr_core_MacroTile*)TTY_p->Window_p->Tile_p->p)->MacroTabs.pp[i])->MicroWindow);
    ...
    for (int row = Position.y+2, i = 0; row < Position.y+height; ++row, ++i) {
        ...
        if (!Program_p || !Program_p->Prototype_p) {
            for (int j = 0, k = 0; defaultName_p[j] != 0; ++j, k++) {
                Grid_p[row].Glyphs_p[Position.x+k+3] = ttyr_core_getGlyphHelper(defaultName_p[j]);
            }
            continue;
        }
        NH_API_UTF32 title_p[255] = {0};
        ...
        for (int j = 0, k = 0; title_p[j] != 0; ++j, k++) {
            Grid_p[row].Glyphs_p[Position.x+k+3] = ttyr_core_getGlyphHelper(title_p[j]);
        }
        if (((ttyr_core_MacroTile*)TTY_p->Window_p->Tile_p->p)->current == i) {
            Grid_p[row].Glyphs_p[Position.x+1] = ttyr_core_getGlyphHelper(0x25cf);
        }
    }
    ...
    ttyr_core_Row *Grid_p)
{
    ...
    for (int i = 0; i < TTY_p->Windows.size; ++i) {
        ttyr_core_Program *Program_p = ttyr_core_getCurrentProgram(&TTYR_CORE_MACRO_TAB(((ttyr_core_MacroWindow*)TTY_p->Windows.pp[i])->Tile_p)->MicroWindow);
        if (!Program_p || !Program_p->Prototype_p) {continue;}
        NH_API_UTF32 title_p[255] = {0};
        Program_p->Prototype_p->Callbacks.getTitle_f(Program_p, title_p, 255);
        int length = 0;
