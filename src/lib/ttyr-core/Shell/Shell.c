...
typedef struct ttyr_core_ShellKey{
    NH_API_KEY_E key;
    char *s;
 ...
 */
static ttyr_core_ShellKey KEYS_P[] = {
...
typedef struct ttyr_core_ShellSelection {
    ...
    bool trippleClick;
    bool moved;
} ttyr_core_ShellSelection;

typedef struct ttyr_core_ShellScroll {
    nh_api_PixelPosition Position;
    nh_api_PixelPosition Current;
    bool active;
    nh_core_SystemTime LastScroll;
} ttyr_core_ShellScroll;

typedef struct ttyr_core_Shell {
    ttyr_core_ShellSocket Socket;
    ttyr_core_ShellSelection Selection;
    ttyr_core_ShellScroll Scroll;
    ST *ST_p;
    ttyr_core_Row *Rows_p;
    int ttyfd;
    fd_set rfd;
...
static void ttyr_core_getShellColor(
    uint32_t color, ttyr_core_Color *Color_p)
{
    if (IS_TRUECOL(color)) {
...
static void ttyr_core_copyShellRow(
    ttyr_core_Shell *Shell_p, Line line, int cols, ttyr_core_Row *Row_p)
{
    ...
    for (int i = 0; i < cols && i < Shell_p->ST_p->col; ++i) 
    {
        ...
        if (line[i].fg != defaultfg) {
            ...
            ttyr_core_getShellColor(line[i].fg, &Row_p->Glyphs_p[i].Foreground.Color);
        } else {
        ...
        if (line[i].bg != defaultbg) {
            ...
            ttyr_core_getShellColor(line[i].bg, &Row_p->Glyphs_p[i].Background.Color);
        } else {
...
static void ttyr_core_drawShell(
    ttyr_core_Shell *Shell_p)
{
    ...
    for (int row = 0, copyRow = ST_p->scrollup-Shell_p->scroll; historyLength && row < historyLength; row++, copyRow++) {
        ttyr_core_copyShellRow(Shell_p, ST_p->history[copyRow], ST_p->historyCols[copyRow], Shell_p->Rows_p+row);
    }
    for (int y = 0; y < (ST_p->row-historyLength); y++) {
        ttyr_core_copyShellRow(Shell_p, ST_p->line[y], ST_p->col, Shell_p->Rows_p+y+historyLength);
    }
...
static TTYR_CORE_RESULT ttyr_core_handleFastScroll(
    ttyr_core_Shell *Shell_p)
{
...
static TTYR_CORE_RESULT ttyr_core_updateShell(
    ttyr_core_Program *Program_p)
{
    ttyr_core_Shell *Shell_p = Program_p->handle_p;
    if (!Shell_p->ST_p) {return TTYR_CORE_SUCCESS;}
 
    TTYR_CHECK(ttyr_core_handleShellSocket(&Shell_p->Socket))

    if (Shell_p->Scroll.active){ 
        TTYR_CHECK(ttyr_core_handleFastScroll(Shell_p))
    }
...
static void ttyr_core_resetShellSelectionBuffer(
    ttyr_core_Shell *Shell_p)
{
...
static TTYR_CORE_RESULT ttyr_core_copyFromShell(
    ttyr_core_Shell *Shell_p)
{
    ...
    ttyr_core_resetShellSelectionBuffer(Shell_p);
    memset(&Shell_p->Selection, 0, sizeof(ttyr_core_ShellSelection));
...
static TTYR_CORE_RESULT ttyr_core_handleShellSelection(
    ttyr_core_Shell *Shell_p)
{
    ttyr_core_resetShellSelectionBuffer(Shell_p);
...
static char *ttyr_core_getShellKey(
    ttyr_core_Shell *Shell_p, NH_API_KEY_E k, uint state)
{
    for (ttyr_core_ShellKey *kp = KEYS_P; kp < KEYS_P + sizeof(KEYS_P)/sizeof(KEYS_P[0]); kp++) {
        if (kp->key != k)
                continue;
...
static TTYR_CORE_RESULT ttyr_core_sendMouseEvent(
    ttyr_core_Shell *Shell_p, nh_api_MouseEvent Event)
{
...
static TTYR_CORE_RESULT ttyr_core_handleShellInput(
    ttyr_core_Program *Program_p, nh_api_WSIEvent Event)
{
    ...
    if (Event.type == NH_API_WSI_EVENT_KEYBOARD && Event.Keyboard.trigger == NH_API_TRIGGER_PRESS) {
        ...
        if ((specialkey = ttyr_core_getShellKey(Shell_p, Event.Keyboard.special, Event.Keyboard.state)) && Event.Keyboard.codepoint == 0) {
            if (specialkey) {
                ttywrite(Shell_p->ST_p, specialkey, strlen(specialkey), 1);
            }
        } else if (PASTE_KEY) {
            ttyr_core_pasteIntoShell(Shell_p);            

        } else if (COPY_KEY) {
            ttyr_core_copyFromShell(Shell_p);            
    ...
    if (Event.type == NH_API_WSI_EVENT_MOUSE) {
        ...
        if (Event.Mouse.type == NH_API_MOUSE_LEFT && Event.Mouse.trigger == NH_API_TRIGGER_PRESS) {
            ...
            if (Shell_p->Selection.doubleClick || Shell_p->Selection.trippleClick) {
                ttyr_core_handleShellSelection(Shell_p);
            }
        }
        ...
        if (Shell_p->ST_p->mode & TERM_MODE_ALTSCREEN) {
            ttyr_core_sendMouseEvent(Shell_p, Event.Mouse);
        } else {
...
static TTYR_CORE_RESULT ttyr_core_getShellCursor(
    ttyr_core_Program *Program_p, int *x_p, int *y_p)
{
    if (((ttyr_core_Shell*)Program_p->handle_p)->scroll != 0) {return TTYR_CORE_SUCCESS;}

    // MODE_HIDE is activated by for example: echo -e "\e[?25l"
    if (((ttyr_core_Shell*)Program_p->handle_p)->ST_p->windowMode & MODE_HIDE) {
        *x_p = -1;
        *y_p = -1;
    }

    // ST_p is created late during first draw, which might not have happened before this is called.
    else if (((ttyr_core_Shell*)Program_p->handle_p)->ST_p) {
        *x_p = ((ttyr_core_Shell*)Program_p->handle_p)->ST_p->ocx;
        *y_p = ((ttyr_core_Shell*)Program_p->handle_p)->ST_p->ocy;
    }
...
static TTYR_CORE_RESULT ttyr_core_resizeShellIfRequired(
    ttyr_core_Shell *Shell_p, int width, int height)
{
...
static TTYR_CORE_RESULT ttyr_core_startShellIfRequired(
    ttyr_core_Shell *Shell_p)
{
    if (Shell_p->ST_p) {
        return TTYR_CORE_SUCCESS;
    ...
    TTYR_CHECK(ttyr_core_createShellSocket(&Shell_p->Socket, Shell_p->ST_p->pid))

    return TTYR_CORE_SUCCESS;
}

static void ttyr_core_drawSelectionIfRequired(
    ttyr_core_Shell *Shell_p, ttyr_core_Glyph *Glyphs_p, int width, int row)
{
    int index1 = Shell_p->Selection.Start.y - Shell_p->Selection.startScroll;
    int index2 = Shell_p->Selection.Stop.y - Shell_p->Selection.stopScroll;
    ...
    if (Shell_p->Selection.draw && row - Shell_p->scroll >= start && row - Shell_p->scroll <= stop) {
        for (int i = 0; i < width; ++i) {
            ...
            if (Shell_p->Selection.Start.x != Shell_p->Selection.Stop.x || start != stop || Glyphs_p[i].codepoint != ' ') {
                ...
                Glyphs_p[i].mark |= TTYR_CORE_MARK_ACCENT;
            }
        }
    }
}

static TTYR_CORE_RESULT ttyr_core_drawShellRow(
    ttyr_core_Program *Program_p, ttyr_core_Glyph *Glyphs_p, int width, int height, int row)
{
    ttyr_core_Shell *Shell_p = Program_p->handle_p;

    TTYR_CHECK(ttyr_core_resizeShellIfRequired(Shell_p, width, height))
    TTYR_CHECK(ttyr_core_startShellIfRequired(Shell_p))
    
    memcpy(Glyphs_p, Shell_p->Rows_p[row].Glyphs_p, sizeof(ttyr_core_Glyph)*width);

    ttyr_core_drawSelectionIfRequired(Shell_p, Glyphs_p, width, row);

    return TTYR_CORE_SUCCESS;
}

static TTYR_CORE_RESULT ttyr_core_drawShellTopbar(
...
static TTYR_CORE_RESULT ttyr_core_getShellTitle(
    ttyr_core_Program *Program_p, NH_API_UTF32 *title_p, int length)
{
    ttyr_core_Shell *Shell_p = Program_p->handle_p;
...
typedef enum TTYR_CORE_SHELL_COMMAND_E {
    TTYR_CORE_SHELL_COMMAND_COPY = 0,
    TTYR_CORE_SHELL_COMMAND_PASTE,
    TTYR_CORE_SHELL_COMMAND_E_COUNT,
} TTYR_CORE_SHELL_COMMAND_E;

static TTYR_CORE_RESULT ttyr_core_handleShellCommand(
    ttyr_core_Program *Program_p)
{
    ttyr_core_Shell *Shell_p = Program_p->handle_p;
    ...
    {
        case TTYR_CORE_SHELL_COMMAND_COPY:
            ttyr_core_copyFromShell(Program_p->handle_p);
            break;
        case TTYR_CORE_SHELL_COMMAND_PASTE:
            ttyr_core_pasteIntoShell(Program_p->handle_p);
            break;
    }
...
static void ttyr_core_destroyShell(
    void *handle_p)
{
    ttyr_core_Shell *Shell_p = handle_p;
...
static void ttyr_core_destroyShellPrototype(
...
ttyr_core_Interface *ttyr_core_createShellInterface()
{
    ...
    Prototype_p->Callbacks.init_f = ttyr_core_initShell;
    Prototype_p->Callbacks.draw_f = ttyr_core_drawShellRow;
    Prototype_p->Callbacks.drawTopbar_f = ttyr_core_drawShellTopbar;
    Prototype_p->Callbacks.getTitle_f = ttyr_core_getShellTitle;
    Prototype_p->Callbacks.handleInput_f = ttyr_core_handleShellInput;
    Prototype_p->Callbacks.handleCommand_f = ttyr_core_handleShellCommand;
    Prototype_p->Callbacks.getCursorPosition_f = ttyr_core_getShellCursor;
    Prototype_p->Callbacks.update_f = ttyr_core_updateShell;
    Prototype_p->Callbacks.destroyPrototype_f = ttyr_core_destroyShellPrototype;
    Prototype_p->Callbacks.destroy_f = ttyr_core_destroyShell;
