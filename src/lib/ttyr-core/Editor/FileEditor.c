...
{
    while (1) {
        ttyr_core_File *File_p = nh_core_getFromLinkedList(&FileEditor_p->Files, 0);
        if (!File_p) {break;}
        ttyr_core_closeFile(FileEditor_p, File_p);
    }
...
static TTYR_CORE_RESULT ttyr_core_renderFileEditor(
    ttyr_core_FileEditor *FileEditor_p)
{
    for (int i = 0; i < FileEditor_p->Files.count; ++i) {
        ttyr_core_File *File_p = nh_core_getFromLinkedList(&FileEditor_p->Files, i);
        TTYR_CHECK_NULL(File_p)
        TTYR_CHECK(ttyr_core_renderFile(File_p))
    }
...
static bool ttyr_core_hasFile(
    ttyr_core_FileEditor *Editor_p, ttyr_core_File *File_p)
{
    bool hasFile = false;

    for (int i = 0; i < Editor_p->Files.count; ++i) {
        ttyr_core_File *Compare_p = nh_core_getFromLinkedList(&Editor_p->Files, i);
        if (File_p == Compare_p) {
            hasFile = true;
            break;
        }
    ...
    ttyr_core_Program *Program_p, ttyr_core_TreeListingNode *Node_p, bool readOnly)
{
    ...
    switch (File_p->type) 
    {
        case TTYR_CORE_FILE_TEXT :
            File_p->handle_p = ttyr_core_openTextFile(&Node_p->Path); 
            break;
    ...
    TTYR_CHECK_2(NULL, ttyr_core_createFileViews(FileEditorView_p, File_p))

    nh_core_prependToLinkedList(&FileEditor_p->Files, File_p);
    FileEditor_p->current = 0;
    Node_p->File_p = File_p;

    TTYR_CHECK_2(NULL, ttyr_core_renderFileEditor(FileEditor_p))

    TTYR_CHECK_2(NULL, ttyr_core_setCustomSuffixMessage(
...
TTYR_CORE_RESULT ttyr_core_closeFile(
    ttyr_core_FileEditor *Editor_p, ttyr_core_File *File_p)
{
    if (File_p == NULL || !ttyr_core_hasFile(Editor_p, File_p)) {
    ...
    {
        case TTYR_CORE_FILE_TEXT :
            ttyr_core_closeTextFile(File_p->handle_p);
            break;
    }
    ...
    ttyr_core_Program *Program_p, NH_API_UTF32 c)
{
    ...
    switch (c)
    {
        case 'f' :
        {
            ...
            // Handle view.
            ttyr_core_EditorView *View_p = &((ttyr_core_Editor*)Program_p->handle_p)->View;
    ...
    if (File_p != NULL) {
        ...
        // Update message.
        ttyr_core_setCustomSuffixMessage(NULL, TTYR_CORE_MESSAGE_EDITOR_FILE_EDIT, File_p->Node_p->Path.p, File_p->Node_p->Path.length);
    }
    ...
    ttyr_core_Program *Program_p, NH_API_UTF32 c)
{
    ...
    ttyr_core_FileEditor *FileEditor_p = &((ttyr_core_Editor*)Editor_p)->FileEditor;
    ttyr_core_File *File_p = nh_core_getFromLinkedList(&FileEditor_p->Files, FileEditor_p->current);

    if (File_p != NULL) {
        TTYR_CHECK(ttyr_core_handleFileInput(Program_p, File_p, c, Editor_p->insertMode, &Program_p->refresh))
...
static TTYR_CORE_RESULT ttyr_core_drawHelp(
    ttyr_core_Glyph *Glyphs_p, int width, int line, int lines, int scroll)
{
    line += scroll;
...
static TTYR_CORE_RESULT ttyr_core_updateFileEditorView(
    ttyr_core_FileEditor *FileEditor_p, ttyr_core_EditorView *View_p, int width)
{
    ...
    TTYR_CHECK(ttyr_core_updateFileViews(View_p))
...
{
    ttyr_core_Editor *Editor_p = Program_p->handle_p;
    ttyr_core_FileEditor *FileEditor_p = &Editor_p->FileEditor;
    ttyr_core_FileEditorView *View_p = &Editor_p->View.FileEditor;

    TTYR_CHECK(ttyr_core_updateFileEditorView(FileEditor_p, &Editor_p->View, width))

    if (FileEditor_p->Files.count == 0) {
        TTYR_CHECK(ttyr_core_drawHelp(Glyphs_p, View_p->width, row, height, View_p->helpScroll))
    }

    for (int i = View_p->currentOffset, width2 = 0, count = 0; i < FileEditor_p->Files.count; ++i, ++count) {
        if (count == View_p->maxOnScreen) {break;}
        ttyr_core_File *File_p = nh_core_getFromLinkedList(&FileEditor_p->Files, i);
        ttyr_core_FileView *FileView_p = ttyr_core_getFileView(&Editor_p->View, File_p);
        TTYR_CHECK_NULL(File_p)
        TTYR_CHECK_NULL(FileView_p)
        TTYR_CHECK(ttyr_core_drawFileRow(Program_p, File_p, FileView_p, Glyphs_p+width2, row))
