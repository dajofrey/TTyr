...
TTYR_TTY_END(ttyr_tty_createTile(Tile_p, TTYR_TTY_TILE_TYPE_MICRO, Parent_p, index))
}
...
TTYR_TTY_BEGIN()
    ...
    TTYR_TTY_CHECK_MEM_2(NULL, Tab_p)
...
TTYR_TTY_BEGIN()
    ...
    TTYR_TTY_CHECK_MEM_2(NULL, Tabs_p)
    ...
    *Tabs_p = nh_core_initList(8); // Don't change size to TTY_p->Prototypes.size, it might not be initialized.
    ...
    for (int i = 0; i < TTY_p->Prototypes.size; ++i) {
        ...
        TTYR_TTY_CHECK_NULL_2(NULL, Tab_p)
...
TTYR_TTY_BEGIN()
    ...
    TTYR_TTY_CHECK(ttyr_tty_destroyView(NULL, Window_p->View_p))
...
TTYR_TTY_BEGIN()
    ...
    if (!Window_p->Tabs_p || Window_p->Tabs_p->size == 0) {TTYR_TTY_END(TTYR_TTY_SUCCESS)}
...
TTYR_TTY_END(!Tab_p ? NULL : TTYR_TTY_MICRO_TILE(Tab_p->Tile_p)->Program_p)
}
