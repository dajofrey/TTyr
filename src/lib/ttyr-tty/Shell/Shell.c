...
TTYR_TTY_BEGIN()
    ...
    for (int row = 0, copyRow = ST_p->scrollup-Shell_p->scroll; historyLength && row < historyLength; row++, copyRow++) {
        ttyr_tty_copyShellRow(Shell_p, ST_p->history[copyRow], ST_p->historyCols[copyRow], Shell_p->Rows_p+row);
    }
    for (int y = 0; y < (ST_p->row-historyLength); y++) {
        ttyr_tty_copyShellRow(Shell_p, ST_p->line[y], ST_p->col, Shell_p->Rows_p+y+historyLength);
    }
...
TTYR_TTY_BEGIN()
    ...
    memset(&Shell_p->Selection, 0, sizeof(ttyr_tty_ShellSelection));
...
TTYR_TTY_BEGIN()
    ...
    if (Event.type == NH_WSI_EVENT_KEYBOARD && Event.Keyboard.trigger == NH_WSI_TRIGGER_PRESS) {
        ...
        if ((specialkey = ttyr_tty_getShellKey(Shell_p, Event.Keyboard.special, Event.Keyboard.state)) && Event.Keyboard.codepoint == 0) {
            ...
            ttyr_tty_copyFromShell(Shell_p);            
...
TTYR_TTY_BEGIN()
    ...
    Shell_p->Rows_p = nh_core_allocate(sizeof(ttyr_tty_Row)*height);
    memset(Shell_p->Rows_p, 0, sizeof(ttyr_tty_Row)*height);

    for (int row = 0; row < height; ++row) {
        Shell_p->Rows_p[row].Glyphs_p = nh_core_allocate(sizeof(ttyr_tty_Glyph)*width);
        memset(Shell_p->Rows_p[row].Glyphs_p, 0, sizeof(ttyr_tty_Glyph)*width);
    }
...
typedef enum TTYR_TTY_SHELL_COMMAND_E {
    TTYR_TTY_SHELL_COMMAND_COPY = 0,
    TTYR_TTY_SHELL_COMMAND_PASTE,
    TTYR_TTY_SHELL_COMMAND_E_COUNT,
...
TTYR_TTY_BEGIN()
    ...
    {
        case TTYR_TTY_SHELL_COMMAND_COPY:
            ttyr_tty_copyFromShell(Program_p->handle_p);
            break;
        case TTYR_TTY_SHELL_COMMAND_PASTE:
            ttyr_tty_pasteIntoShell(Program_p->handle_p);
            break;
    }
...
TTYR_TTY_BEGIN()
    ...
    Prototype_p->Callbacks.update_f = ttyr_tty_updateShell;
